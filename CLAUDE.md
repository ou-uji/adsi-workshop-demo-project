...
- **次はバグ③**（ログアウト後に画面遷移しない ※SageMaker 都合で難しい）。ヒント: ログアウト処理後のリダイレクト有無。バグ①②と同じ流れ（Issue 起票 → Plan mode 調査 → 壁打ち → TDD → PR）で進める想定。
- ワークフロー確立済み: **専用ブランチ `bug-fix/<name>` を切る → TDD → PR（`Closes #<issue>`）→ ユーザーがマージ → main 同期 + ブランチ削除**。process 記録は `process/YYYYMMDD/` に残す。
