...
## ハンズオン3: 新規機能「有給休暇申請」— 🚧 進行中（2026-07-13）

> スライド⑤「大きめの機能を追加してみよう」。お題は **有給休暇申請機能**。
> 既存の **勤怠修正（correction / Unit 05）** と同じ「申請 → 承認/却下 → 反映」の型を、**SDD 工程スキルにプロセスを任せて**再現する体験。
> **研修用デモのため短時間で作る・完成しなくても OK**。今回は **ユーザーレビューなしで NonStop で PR 作成まで**進める方針（ただし AI 自身のレビュー＋テスト検証は飛ばさず、CRITICAL エラー・デグレ無しを確認して進む）。
...
| `新規機能_有給休暇申請_レビュー結果.md` | AI 自己レビュー結果 | 🚧 |
...
- 🚧 設計（`/design`）→ テスト仕様 → TDD 実装 → 検証 → 自己レビュー → PR 作成（NonStop 実行中）
